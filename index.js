<div className="main justify-center  ">
              <div class="section-content flex  ">


              <div class="central-content p-40 w-3/6  ">
                <div class="font-semibold text-5xl">The Best Logistics Service Provider</div>
                <div  class="mt-3.5 text-2xl font-semibold">We are flexible and delicated Logistics Service Partner. We work with you to design custom solutions that add value to your business </div>
                <div class="button-section border-4 rounded-3xl w-40 p-2   mt-4 	ml-32">
                    <button class="button text-xl font-black font-bold	" onClick={()=>goToService()}>Our Services</button>
                </div>

            </div>


            <div class="w-1/2 flex justify-center m-8 ">
             <img  src="https://res.cloudinary.com/dcqew2kqy/image/upload/v1637749610/20945966_j7it1g.jpg" alt="group-chat" class=" m-4 h-2/3 mt-20" />
            </div>

           
        </div>

        

        </div>
https://res.cloudinary.com/dcqew2kqy/image/upload/v1647325584/letter-initial-e-shopping-bag-for-online-digital-retail-e-commerce-shop-store-market-supermarket-company-brand-mobile-app-simple-creative-flat-logo-2G41XFC_cnofab.jpg
